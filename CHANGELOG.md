...
- **Pre-commit required:** **[`.pre-commit-config.yaml`](.pre-commit-config.yaml)** — **`fail_fast: true`**; **`pre-commit-hooks`** **v6.0.0** (whitespace, EOF, **`check-yaml`** with **`.github/workflows/`** excluded, JSON, merge conflicts, **`check-added-large-files`** **`--maxkb=500`**, line endings, private keys, **`debug-statements`**) plus **Gitleaks** (first) + **Black** (format) + **Ruff** (lint, **`--fix`**) (no **`ruff-format`** — conflicts with **Black**). README / **`MAINTAINERS.md`**: bump hook **`rev:`** (**Black** / **Ruff** / **Gitleaks**); align **Gitleaks** hook with **`ci.yml`** **`gitleaks_version`** when desired; **`pre-commit autoupdate`** for **pre-commit-hooks**.
...
- **`python-pr-suite.yml`:** forwards **`run_pre_commit`** (default **`true`**), **`run_gitleaks`** (default **`true`**); no **`run_ruff`** / **`ruff_version`**. Expanded forwarding for **PR/push parity** in typical apps (see workflow file). **`COMPANY_RUNBOOK.md`:** **Residual gaps (not oversights)** table. **`README`:** link to residual section.
