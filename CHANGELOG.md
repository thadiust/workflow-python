...
- **Pre-commit required:** **[`.pre-commit-config.yaml`](.pre-commit-config.yaml)** (**Ruff** **0.15.9**, **Gitleaks** **8.18.4**) and **[`.github/workflows/pre-commit.yml`](.github/workflows/pre-commit.yml)** (**`pre-commit run --all-files`** on push/PR). README / **`MAINTAINERS.md`**: bump hook **`rev:`** when **`ci.yml`** tool defaults change.
