...
- **Pre-commit required:** **[`.pre-commit-config.yaml`](.pre-commit-config.yaml)** — **`fail_fast: true`**; **`pre-commit-hooks`** **v6.0.0** (whitespace, EOF, **`check-yaml`** with **`.github/workflows/`** excluded, JSON, merge conflicts, line endings, private keys, **`debug-statements`**) plus **Gitleaks** (first) + **Black** (format) + **Ruff** (lint, **`--fix`**) (no **`ruff-format`** — conflicts with **Black**). CI: **`pre-commit-check`** only for **Gitleaks** (no **`gitleaks-scan`** job). README / **`MAINTAINERS.md`**: bump hook **`rev:`** (**Black** / **Ruff** / **Gitleaks**); **`pre-commit autoupdate`** for **pre-commit-hooks**.
...
- **`ci.yml`:** removed **`gitleaks-scan`** job and **`run_gitleaks`** / **`gitleaks_version`** inputs — **Gitleaks** runs **only** inside **`pre-commit-check`** (pre-commit hook). **`python-pr-suite.yml`** and **sample-python-app** callers updated. **Breaking:** callers must drop **`run_gitleaks`** / **`gitleaks_version`** from **`with:`**; use **`.pre-commit-config.yaml`** for **Gitleaks** version. No **Gitleaks** SARIF from **`ci.yml`** (add your own **`secrets-gitleaks`** workflow if required).
...
- **`python-pr-suite.yml`:** forwards **`run_pre_commit`** (default **`true`**); no **`run_ruff`** / **`ruff_version`**; no **`run_gitleaks`** (**Gitleaks** is pre-commit-only). Expanded forwarding for **PR/push parity** in typical apps (see workflow file). **`COMPANY_RUNBOOK.md`:** **Residual gaps (not oversights)** table. **`README`:** link to residual section.
