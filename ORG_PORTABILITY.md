...
After mechanical replace, run **[`scripts/refresh-pip-constraints.sh`](scripts/refresh-pip-constraints.sh)** whenever you bump **`pytest_version`**, default **`pip_tools_version`**, or the **PyYAML** version used by **`install-pyyaml-hashed`**, or when you change the **Ruff** version in **[`.github/actions/ruff`](.github/actions/ruff/README.md)** (if you use that composite), then commit the updated **`constraints*.txt`** files. Do the same in forked **`sast-bandit`** and **`pip-audit-scan-action`** using their **`scripts/refresh-pip-constraints.sh`** when you change default **Bandit** / **pip-audit** versions. If you skip this after a bump, CI still runs but composites may log **`::warning`** and install **without** **`--require-hashes`** until matching constraint files exist.
