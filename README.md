...
1. **[Ruff](https://docs.astral.sh/ruff/)** via the composite at [`.github/actions/ruff`](.github/actions/ruff/README.md). [`ci.yml`](.github/workflows/ci.yml) references it as **`./.github/actions/ruff`** (resolved from this repository when the reusable workflow runs). **Callers** only reference the **reusable workflow** (`uses: org/workflow-python/.github/workflows/ci.yml@…`); they must not use `./` to reach composites in another repo. `ruff check` (GitHub annotations) and optional `ruff format --check`, pinned version, **`--force-exclude`**. Toggle with `run_ruff` and related inputs.
2. **[pytest](https://pytest.org/)** via [`.github/actions/pytest`](.github/actions/pytest/README.md) (`./.github/actions/pytest` in `ci.yml`), **in parallel** with Ruff when both are enabled (`run_pytest`). Installs deps from `pytest_requirements_file` and runs `python -m pytest` with `pytest_args`; pytest itself is pinned to `pytest_version`.
