...
- **Supply chain:** **Consumers should reference this workflow by semver tag** (for example **`@v1.0.8`**) so upgrades are explicit and reproducible. This project does not require pinning to commit SHAs. **`@main` is intended for development/testing**, not the default for app repositories. **Note:** Each job’s **`uses:`** resolves independently. On **semver tags**, **Ruff** and **pytest** use the **same tag** as **`ci.yml`**. On branch **`main`**, **Ruff** and **pytest** use **`@main`** so CI exercises the latest local composites. **`secrets-gitleaks`** is pinned at **`@v1.0.3`**; **`sast-bandit`** at **`@v1.0.2`**; **`pip-audit-scan-action`** at **`@v1.0.1`** (see [`ci.yml`](.github/workflows/ci.yml)). **`upload_code_scanning`** (default **`true`**) uploads **Gitleaks** and **Bandit** SARIF via **`github/codeql-action/upload-sarif`** — caller workflows need **`permissions: security-events: write`** on the job that **`uses`** this workflow (see example). Upload steps use **`continue-on-error: true`** so missing **GitHub Advanced Security** does not fail the pipeline.
