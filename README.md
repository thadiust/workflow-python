...
3. **[Trivy](https://github.com/aquasecurity/trivy)** (**Wave 1** repository **fs** + **config**) via [`thadiust/trivy-scan`](https://github.com/thadiust/trivy-scan) (**`uses: …/trivy-scan@main`** on branch **main**). Job **`trivy-repo-scan`** **`needs`** **Ruff** and **pytest** (same gate as **Gitleaks**), so it runs **in parallel with Gitleaks** once lint/tests are green. Optional **`docker-build`** **`needs`** **Ruff**, **pytest**, **`gitleaks-scan`**, and **`trivy-repo-scan`** (no image build if **Gitleaks** fails). **`trivy-image-scan`** **`needs`** **`docker-build`** only — a **leaf**. Toggle with **`run_trivy`**, **`run_docker_build`**, **`run_trivy_image_scan`**, and **`trivy_*`** inputs.
...
This layout runs **Ruff** and **pytest** first; then **Gitleaks** and **Trivy repo** in parallel; then **Bandit** and **pip-audit** (parallel with each other). **Docker build** (optional) runs only after **Gitleaks** and **Trivy repo** both succeed (and **Ruff**/**pytest** gates); **Trivy image** (optional) runs after **Docker build** and is **not** a dependency for anything else.
...
- **Tests before security scans (deliberate):** **Gitleaks**, **Trivy repo**, **Bandit**, and **pip-audit** run only after **Ruff** and **pytest** succeed (or those jobs are skipped via toggles). If **pytest fails**, they **do not run**. **Docker build** waits on **Gitleaks** and **Trivy repo** (no image if secrets fail; repo scan still completes before build when **`run_trivy`** is on). **Trivy image** is a separate **leaf** job and does not block Bandit, pip-audit, or Gitleaks.
