...
1. **[pre-commit](https://pre-commit.com/)** (**`pre-commit run --all-files`**) via **`pre-commit/action`** — uses the **caller** repo’s **`.pre-commit-config.yaml`** (**Ruff**, **ruff-format**, **Gitleaks** hook, **pre-commit-hooks**, etc.). Install hooks locally with **`pre-commit install`**; run **`pre-commit run --all-files`** before push. See **[`.pre-commit-config.yaml`](.pre-commit-config.yaml)** as a template. Toggle **`run_pre_commit`**. There is **no** separate **Ruff** Actions job — lint/format are **only** via pre-commit in **`ci.yml`**. For a standalone Ruff composite (custom workflows), use **[`.github/actions/ruff`](.github/actions/ruff/README.md)** directly.
2. **[Gitleaks](https://github.com/gitleaks/gitleaks)** via [`thadiust/secrets-gitleaks`](https://github.com/thadiust/secrets-gitleaks) (**`…/secrets-gitleaks@main`**). **Runs in parallel with** **pre-commit** and does **not** wait on **pytest** — **full-history** scan (**`fetch-depth: 0`**) before slow tests. **Pre-commit** may also run the **Gitleaks** hook (working tree); keep both if you want **history** in CI regardless of hook behavior. See [**Removed the file, but CI still fails?**](https://github.com/thadiust/secrets-gitleaks/blob/main/README.md#removed-the-file-but-ci-still-fails). Toggle **`run_gitleaks`**.
...
**DAG summary:** **pre-commit ∥ Gitleaks** → **pytest** → **Trivy repo ∥ Bandit ∥ pip-audit** → optional **Docker** → optional **Trivy image**.
...
|-----|----------------|----------------|
| pre-commit | **Hooks** (whitespace, YAML/JSON, **Ruff** / **ruff-format**, Gitleaks hook, etc.) | hook failures in log |
...
- **Fast-then-slow (deliberate):** **Gitleaks** does **not** wait on **pytest**; it runs with **pre-commit** in the first wave so secrets and hook policy (including **Ruff** via pre-commit) are caught before slow installs/tests. **pytest** waits on **pre-commit** + **Gitleaks**. **Trivy repo**, **Bandit**, and **pip-audit** wait on those + **pytest**, then run **in parallel**. **Docker build** waits on all of those (each **skipped** only when its toggle is off). **Trivy image** is a **leaf**.
