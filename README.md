...
2. **[Gitleaks](https://github.com/gitleaks/gitleaks)** via [`thadiust/secrets-gitleaks`](https://github.com/thadiust/secrets-gitleaks) (**`…/secrets-gitleaks@main`**). **`gitleaks-scan`** runs **in parallel with** **`pre-commit-check`** when both are on (**both are prechecks** before **pytest**). The job is **full-history** (**`fetch-depth: 0`**); the **Gitleaks** hook in pre-commit still scans the **working tree**. See [**Removed the file, but CI still fails?**](https://github.com/thadiust/secrets-gitleaks/blob/main/README.md#removed-the-file-but-ci-still-fails). Toggle **`run_gitleaks`**.
...
**DAG summary:** **pre-commit-check ∥ gitleaks-scan** (prechecks) → **pytest** → **Trivy repo ∥ Bandit ∥ pip-audit** → optional **Docker** → optional **Trivy image**.
...
- **Fast-then-slow (deliberate):** **`pre-commit-check`** and **`gitleaks-scan`** run **in parallel** when both are enabled (**precheck** wave: hooks + full-history secrets). **pytest** waits on **both** (each may be **skipped** only when its toggle is off). **Trivy repo**, **Bandit**, and **pip-audit** wait on those + **pytest**, then run **in parallel**. **Docker build** waits on all of those (each **skipped** only when its toggle is off). **Trivy image** is a **leaf**.
- **Parallel jobs vs minutes:** **`runner-info`** logs **`runner.*`** once (toggle **`run_runner_info: false`** to drop that job). **pre-commit** and **Gitleaks** can run together; then **pytest**; then up to **three** scanners in parallel. Optional **Docker** / **Trivy image** add runners. Disable jobs via inputs to save minutes.
