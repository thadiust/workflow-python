...
1. **[pre-commit](https://pre-commit.com/)** (**`pre-commit run --all-files`**) via **`pre-commit/action`** — uses the **caller** repo’s **`.pre-commit-config.yaml`** (install hooks locally with **`pre-commit install`**; run **`pre-commit run --all-files`** before push). Default hooks align with **Ruff**, **Gitleaks**, and **pre-commit-hooks** (see **[`.pre-commit-config.yaml`](.pre-commit-config.yaml)** in this repo as a template). Toggle **`run_pre_commit`**.
2. **Optional [Ruff](https://docs.astral.sh/ruff/)** via [`.github/actions/ruff`](.github/actions/ruff/README.md) — **off by default** when **pre-commit** already runs **`ruff`** / **`ruff-format`**. Set **`run_ruff: true`** only if you need the composite action without a pre-commit config. Toggle **`run_ruff`** and **`ruff_*`**.
3. **[Gitleaks](https://github.com/gitleaks/gitleaks)** via [`thadiust/secrets-gitleaks`](https://github.com/thadiust/secrets-gitleaks) (**`…/secrets-gitleaks@main`**). **Runs in parallel with** **pre-commit** / **Ruff** and does **not** wait on **pytest** — **full-history** scan (**`fetch-depth: 0`**) before slow tests. **Pre-commit** may also run the **Gitleaks** hook (working tree); keep both if you want **history** in CI regardless of hook behavior. See [**Removed the file, but CI still fails?**](https://github.com/thadiust/secrets-gitleaks/blob/main/README.md#removed-the-file-but-ci-still-fails). Toggle **`run_gitleaks`**.
4. **[pytest](https://pytest.org/)** via [`.github/actions/pytest`](.github/actions/pytest/README.md) — **`needs`** **`pre-commit-check`**, **`ruff-lint`**, and **`gitleaks-scan`** so **slow tests run only after** those gates (each may be **skipped** only when its toggle is off). Toggle **`run_pytest`**.
5. **[Trivy](https://github.com/aquasecurity/trivy)** (**fs** + **config**) via [`thadiust/trivy-scan`](https://github.com/thadiust/trivy-scan) (**`…/trivy-scan@main`**). **`trivy-repo-scan`** **`needs`** **pre-commit**, **Ruff**, **Gitleaks**, and **pytest** (same pattern as Bandit/pip-audit) — **deliberate** so **tests fail before** heavier scanners spend minutes; for **earliest** fs/IaC signal only, run a separate workflow or fork the DAG. Toggle **`run_trivy`** and **`trivy_*`**.
6. **[Bandit](https://github.com/pycqa/bandit)** and **[pip-audit](https://github.com/pypa/pip-audit)** via **`sast-bandit`** / **`pip-audit-scan-action`** — **`needs`** **pre-commit**, **Ruff**, **Gitleaks**, **pytest**; **in parallel with each other and with Trivy repo** after pytest. Toggle **`run_bandit`**, **`run_pip_audit_scan`**.
7. Optional **`docker-build`** then **`trivy-image-scan`** (leaf): **`docker-build`** **`needs`** **pre-commit**, **Ruff**, **Gitleaks**, **pytest**, **Trivy repo**, **Bandit**, **pip-audit** — image only after that wave (**`run_docker_build`**, **`run_trivy_image_scan`**, **`trivy_image_fail_on_findings`**, etc.).

**Toggle jobs** with **`run_pre_commit`**, **`run_ruff`**, **`run_gitleaks`**, **`run_pytest`**, **`run_trivy`**, **`run_bandit`**, **`run_pip_audit_scan`**, **`run_docker_build`**, **`run_trivy_image_scan`**. If **every** job is disabled, GitHub rejects the run — leave at least one enabled.

**DAG summary:** **pre-commit ∥ Ruff (optional) ∥ Gitleaks** → **pytest** → **Trivy repo ∥ Bandit ∥ pip-audit** → optional **Docker** → optional **Trivy image**.
...
|-----|----------------|----------------|
| pre-commit | **Hooks** (whitespace, YAML/JSON, Ruff, Gitleaks hook, etc.) | hook failures in log |
| Ruff (standalone job) | **Lint / format** | `scan_status`, `format_ok` |
...
- **Fast-then-slow (deliberate):** **Gitleaks** does **not** wait on **pytest**; it runs with **pre-commit** and optional **Ruff** in the first wave so secrets and hook policy are caught before slow installs/tests. **pytest** waits on **pre-commit** + **Ruff** + **Gitleaks**. **Trivy repo**, **Bandit**, and **pip-audit** wait on those + **pytest**, then run **in parallel**. **Docker build** waits on all of those (each **skipped** only when its toggle is off). **Trivy image** is a **leaf**.
- **Parallel jobs vs minutes:** **`runner-info`** logs **`runner.*`** once (toggle **`run_runner_info: false`** to drop that job). **pre-commit**, **Ruff**, and **Gitleaks** can run together; then **pytest**; then up to **three** scanners in parallel. Optional **Docker** / **Trivy image** add runners. Disable jobs via inputs to save minutes.
- **Timeouts:** **pre-commit** and **Ruff** use **`timeout-minutes: 15`**; other jobs use **`timeout-minutes: 30`** so a hung scanner does not burn the runner default (6 hours).
...
| `python_version` | string | `3.11` | Python version for pre-commit, Ruff, Bandit, and pip-audit jobs (Trivy is a standalone binary; this input does not affect Trivy). |
...
| `run_pre_commit` | boolean | `true` | If `false`, the **pre-commit** job is skipped (not recommended — you lose the shared hook gate unless you replace it). |
| `run_ruff` | boolean | `false` | If `true`, run the standalone **Ruff** composite action in addition to (or instead of) **pre-commit** Ruff hooks. |
...
| `run_docker_build` | boolean | `false` | If `true`, build a Docker image only after **pre-commit**, **Ruff** (if enabled), **Gitleaks**, **pytest**, **Trivy repo**, **Bandit**, and **pip-audit** have each succeeded or been skipped (toggle off), then upload a tarball for **`trivy-image-scan`**. |
...
jobs:
  security:
    ...
    with:
      ...
      python_version: "3.11"
      fail_on_vuln: true
      # run_pre_commit: true   # default; requires .pre-commit-config.yaml at repo root
      # run_ruff: true         # optional duplicate if you need the Ruff composite without hooks
      run_pytest: true
