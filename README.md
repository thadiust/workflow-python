...
3. **[Trivy](https://github.com/aquasecurity/trivy)** (**Wave 1** repository **fs** + **config**) via [`thadiust/trivy-scan`](https://github.com/thadiust/trivy-scan) (**`uses: …/trivy-scan@main`**). Job **`trivy-repo-scan`** **`needs`** **Ruff** and **pytest** (same gate as **Gitleaks**), so it runs **in parallel with Gitleaks** once lint/tests are green. Optional **`docker-build`** **`needs`** those plus **`trivy-repo-scan`** (repo scan finishes before the image is built). **`trivy-image-scan`** **`needs`** **`docker-build`** only — a **leaf**: Bandit, pip-audit, and Gitleaks **do not** wait on it. Toggle with **`run_trivy`**, **`run_docker_build`**, **`run_trivy_image_scan`**, and **`trivy_*`** inputs.
