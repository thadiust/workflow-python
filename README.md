...
1. **[pre-commit](https://pre-commit.com/)** (**`pre-commit run --all-files`**) via **`pre-commit/action`** — uses the **caller** repo’s **`.pre-commit-config.yaml`** (**Gitleaks** hook first, then **pre-commit-hooks**, **Black**, **`ruff`** with **`--fix`**, etc.; **`fail_fast: true`**). Install hooks locally with **`pre-commit install`**; run **`pre-commit run --all-files`** before push. See **[`.pre-commit-config.yaml`](.pre-commit-config.yaml)** as a template. Toggle **`run_pre_commit`**. There is **no** separate **Ruff** Actions job — lint/format are **only** via pre-commit in **`ci.yml`**. For a standalone Ruff composite (custom workflows), use **[`.github/actions/ruff`](.github/actions/ruff/README.md)** directly.
2. **[Gitleaks](https://github.com/gitleaks/gitleaks)** via [`thadiust/secrets-gitleaks`](https://github.com/thadiust/secrets-gitleaks) (**`…/secrets-gitleaks@main`**). **`gitleaks-scan`** runs **after** **`pre-commit-check`** when pre-commit is enabled (the **Gitleaks** hook already scans the tree in pre-commit; the job adds **full-history** **`fetch-depth: 0`** before slow tests). If **`run_pre_commit`** is off, **`gitleaks-scan`** still runs when **`run_gitleaks`** is on. See [**Removed the file, but CI still fails?**](https://github.com/thadiust/secrets-gitleaks/blob/main/README.md#removed-the-file-but-ci-still-fails). Toggle **`run_gitleaks`**.
3. **[pytest](https://pytest.org/)** via [`.github/actions/pytest`](.github/actions/pytest/README.md) — **`needs`** **`pre-commit-check`** and **`gitleaks-scan`** so **slow tests run only after** those gates (each may be **skipped** only when its toggle is off). Toggle **`run_pytest`**.
4. **[Trivy](https://github.com/aquasecurity/trivy)** (**fs** + **config**) via [`thadiust/trivy-scan`](https://github.com/thadiust/trivy-scan) (**`…/trivy-scan@main`**). **`trivy-repo-scan`** **`needs`** **pre-commit**, **Gitleaks**, and **pytest** (same pattern as Bandit/pip-audit). Toggle **`run_trivy`** and **`trivy_*`**.
5. **[Bandit](https://github.com/pycqa/bandit)** and **[pip-audit](https://github.com/pypa/pip-audit)** via **`sast-bandit`** / **`pip-audit-scan-action`** — **`needs`** **pre-commit**, **Gitleaks**, **pytest**; **in parallel with each other and with Trivy repo** after pytest. Toggle **`run_bandit`**, **`run_pip_audit_scan`**.
6. Optional **`docker-build`** then **`trivy-image-scan`** (leaf): **`docker-build`** **`needs`** **pre-commit**, **Gitleaks**, **pytest**, **Trivy repo**, **Bandit**, **pip-audit** — image only after that wave (**`run_docker_build`**, **`run_trivy_image_scan`**, **`trivy_image_fail_on_findings`**, etc.).
...
**DAG summary:** **pre-commit** → **Gitleaks** (job) → **pytest** → **Trivy repo ∥ Bandit ∥ pip-audit** → optional **Docker** → optional **Trivy image**.
...
|-----|----------------|----------------|
| pre-commit | **Hooks** (whitespace, YAML/JSON, **Black**, **Ruff** (lint), Gitleaks hook, etc.) | hook failures in log |
...
- **Fast-then-slow (deliberate):** **pre-commit** runs first (hooks include **Gitleaks** on the working tree, **Black**, **Ruff**, etc.). **`gitleaks-scan`** runs **after** **`pre-commit-check`** when both are enabled so CI does not duplicate that wave on two runners. **pytest** waits on **pre-commit** + **Gitleaks** job. **Trivy repo**, **Bandit**, and **pip-audit** wait on those + **pytest**, then run **in parallel**. **Docker build** waits on all of those (each **skipped** only when its toggle is off). **Trivy image** is a **leaf**.
- **Parallel jobs vs minutes:** **`runner-info`** logs **`runner.*`** once (toggle **`run_runner_info: false`** to drop that job). **pre-commit** then **Gitleaks** job (sequential when both on); then **pytest**; then up to **three** scanners in parallel. Optional **Docker** / **Trivy image** add runners. Disable jobs via inputs to save minutes.
...
| `run_pre_commit` | boolean | `true` | If `false`, the **pre-commit** job is skipped (not recommended — you lose Ruff + hooks in **`ci.yml`** unless you replace them). |
...
| `run_docker_build` | boolean | `false` | If `true`, build a Docker image only after **pre-commit**, **Gitleaks**, **pytest**, **Trivy repo**, **Bandit**, and **pip-audit** have each succeeded or been skipped (toggle off), then upload a tarball for **`trivy-image-scan`**. |
