...
3. **[Gitleaks](https://github.com/gitleaks/gitleaks)** via [`thadiust/secrets-gitleaks`](https://github.com/thadiust/secrets-gitleaks) (**`uses: …/secrets-gitleaks@v1.0.0`** on branch **main** — tag releases in that repo to bump). Secrets; **full git history** — checkout uses `fetch-depth: 0`. **`gitleaks-scan`** **`needs`** **`ruff-lint`** and **`pytest-test`**. See [**Removed the file, but CI still fails?**](https://github.com/thadiust/secrets-gitleaks/blob/main/README.md#removed-the-file-but-ci-still-fails) in the action README.
...
3. On **`main`**, follow up with a commit that sets **Ruff** and **pytest** back to **`@main`** so day-to-day CI on this repo still runs the latest local composites (the **tag** remains a frozen snapshot; verify with **`git show v1.x.y:.github/workflows/ci.yml`**). When **`secrets-gitleaks`**, **`sast-bandit`**, or **`pip-audit-scan-action`** ship a new semver tag, update those three **`uses:`** lines and cut a new **`workflow-python`** release if callers should pick it up via **`ci.yml@v…`**.
...
jobs:
  security:
    uses: thadiust/workflow-python/.github/workflows/ci.yml@v1.0.2
    with:
      working_directory: "."
...
For controlled upgrades, call **`uses: …/ci.yml@v1.0.2`** (or another tag) instead of **`@main`**; bump the tag when you intentionally adopt a new release. **`v1.0.2`** pins **Ruff/pytest** and **`thadiust/*`** actions to the versions listed in the tag’s **`ci.yml`** (see supply chain note above).
