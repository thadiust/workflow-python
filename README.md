...
**Lint in this repo:** **[`.pre-commit-config.yaml`](.pre-commit-config.yaml)** includes **[actionlint](https://github.com/rhysd/actionlint)** (workflow semantics under **`.github/workflows/`**) plus a **PyYAML** parse pass for **`.github/actions/**/action.yml`**. Same hooks run in **`ci.yml`** **`pre-commit-check`** and **[`.github/workflows/pre-commit.yml`](.github/workflows/pre-commit.yml)** on **`main`**. **Before every push:** **`pre-commit run --all-files`**. Repos without the full pipeline can still call **`[reusable-actionlint.yml](.github/workflows/reusable-actionlint.yml)`** solo. User-visible changes by tag are listed in **[CHANGELOG.md](CHANGELOG.md)**.
...
**Dependency Review** is **not** part of the **`ci.yml`** DAG. For **PR onboarding**, call **[`python-pr-suite.yml`](.github/workflows/python-pr-suite.yml)** (**Dependency Review** ∥ **`ci.yml`**) — it forwards a **wide** set of **`ci.yml`** inputs (Python, pre-commit, pytest, scanners, lockfile, Docker, Code Scanning, Trivy image gate; defaults match **`ci.yml`**). For inputs **not** on the suite, use a **two-job** caller or copy **[`examples/consumer-pull-request-ci.yml`](examples/consumer-pull-request-ci.yml)**. A **minimal** two-job template is also below. **Residual risks** that are **org/product** choices (fork trust, strict Trivy, second SAST, DR on push) are listed in **[`COMPANY_RUNBOOK.md`](COMPANY_RUNBOOK.md) → Residual gaps**.
...
1. **[pre-commit](https://pre-commit.com/)** (**`pre-commit run --all-files`**) via **`pre-commit/action`** — uses the **caller** repo’s **`.pre-commit-config.yaml`** (**Gitleaks** hook first, then **pre-commit-hooks**, **Black**, **`ruff`** with **`--fix`**, etc.; **`fail_fast: true`**). Toggle **`run_pre_commit`**. There is **no** separate **Ruff** Actions job — use **[`.github/actions/ruff`](.github/actions/ruff/README.md)** only if you want Ruff outside pre-commit.
2. **[Gitleaks](https://github.com/gitleaks/gitleaks)** via [`thadiust/secrets-gitleaks`](https://github.com/thadiust/secrets-gitleaks) — **`gitleaks-scan`** runs **in parallel with** **`pre-commit-check`** when **`run_gitleaks`** is **`true`** (default): **full git history** (**`fetch-depth: 0`**) and optional **SARIF** when **`upload_code_scanning`** is on. The **pre-commit** hook still guards the tree in the same run. Set **`run_gitleaks: false`** to rely on the hook only and save a job. See [**Removed the file, but CI still fails?**](https://github.com/thadiust/secrets-gitleaks/blob/main/README.md#removed-the-file-but-ci-still-fails). Toggle **`run_gitleaks`**, **`gitleaks_version`**.
3. **[pytest](https://pytest.org/)** via [`.github/actions/pytest`](.github/actions/pytest/README.md) — **`needs`** **`pre-commit-check`** and **`gitleaks-scan`** (each may be **skipped** only when its toggle is off). Toggle **`run_pytest`**.
4. **[Trivy](https://github.com/aquasecurity/trivy)** (**fs** + **config**) via [`thadiust/trivy-scan`](https://github.com/thadiust/trivy-scan) (**`…/trivy-scan@main`**). **`trivy-repo-scan`** **`needs`** **pre-commit** and **pytest**. Toggle **`run_trivy`** and **`trivy_*`**.
5. **[Bandit](https://github.com/pycqa/bandit)** and **[pip-audit](https://github.com/pypa/pip-audit)** via **`sast-bandit`** / **`pip-audit-scan-action`** — **`needs`** **pre-commit** and **pytest**; **in parallel with each other and with Trivy repo** after pytest. Toggle **`run_bandit`**, **`run_pip_audit_scan`**.
6. Optional **`docker-build`** then **`trivy-image-scan`** (leaf): **`docker-build`** **`needs`** **pre-commit**, **pytest**, **Trivy repo**, **Bandit**, **pip-audit** — image only after that wave (**`run_docker_build`**, **`run_trivy_image_scan`**, **`trivy_image_fail_on_findings`**, etc.).

**Toggle jobs** with **`run_pre_commit`**, **`run_gitleaks`**, **`run_pytest`**, **`run_trivy`**, **`run_bandit`**, **`run_pip_audit_scan`**, **`run_docker_build`**, **`run_trivy_image_scan`**. If **every** job is disabled, GitHub rejects the run — leave at least one enabled.

**DAG summary:** **pre-commit-check ∥ gitleaks-scan** (when **`run_gitleaks`**) → **pytest** → **Trivy repo ∥ Bandit ∥ pip-audit** → optional **Docker** → optional **Trivy image**.
...
|-----|----------------|----------------|
| pre-commit | **Hooks** (whitespace, YAML/JSON, **Gitleaks** hook, **Black**, **Ruff** (lint), etc.) | hook failures in log |
| Gitleaks (job) | **Secrets** (full history via **`secrets-gitleaks`**; optional **`run_gitleaks: false`**) | `secret_count` |
...
**`ci.yml`** is validated on **GitHub-hosted `ubuntu-latest`** (**linux/amd64**). Nested composites (**[`secrets-gitleaks`](https://github.com/thadiust/secrets-gitleaks)**, **[`trivy-scan`](https://github.com/thadiust/trivy-scan)**, **`sast-bandit`**, **`pip-audit-scan-action`**, etc.) download **Linux x86_64** binaries and use **`jq`** on **`PATH`** where needed for counts. **[`reusable-actionlint.yml`](.github/workflows/reusable-actionlint.yml)** fetches **actionlint** **`linux_amd64`** only. **ARM** or **minimal** runners without **`jq`** require changes to those actions or your runner image before this pipeline is reliable.
...
**`ci.yml`** forwards **tool versions** (**`gitleaks_version`**, **`bandit_version`**, **`pip_audit_version`**) and **`bandit_targets`**. Align **`.pre-commit-config.yaml`** **Gitleaks** hook **`rev:`** with **`gitleaks_version`** when you care about parity. It still **does not** expose every composite knob (e.g. Bandit **`report_format`**, Gitleaks **`baseline_path`**, pip-audit **`report_file`**). For those, **fork** and extend **`workflow_call`** inputs or wrap a job.
...
- **Fast-then-slow (deliberate):** **`pre-commit-check`** and **`gitleaks-scan`** run **in parallel** when both toggles are on (**precheck** wave). **pytest** waits on **both** (each may be **skipped** only when its toggle is off). **Trivy repo**, **Bandit**, and **pip-audit** wait on **pre-commit** + **pytest**, then run **in parallel**. **Docker build** waits on all of those (each **skipped** only when its toggle is off). **Trivy image** is a **leaf**.
- **Parallel jobs vs minutes:** **`runner-info`** logs **`runner.*`** once (toggle **`run_runner_info: false`** to drop that job). **pre-commit** and **Gitleaks** job can run together; then **pytest**; then up to **three** scanners in parallel. Optional **Docker** / **Trivy image** add runners. Disable jobs via inputs to save minutes.
- **Timeouts:** **pre-commit** uses **`timeout-minutes: 15`**; other jobs use **`timeout-minutes: 30`** so a hung scanner does not burn the runner default (6 hours).
- **Supply chain:** On **`workflow-python` branch `main`**, **`ci.yml`** uses **`@main`** for **pytest**, **Trivy**, **`secrets-gitleaks`**, **`sast-bandit`**, and **`pip-audit-scan-action`** (solo-dev floating pins). For reproducible upgrades, **consumers** can call **`ci.yml@v…`** and rely on whatever nested pins that tag records. **`upload_code_scanning`** (default **`true`**) uploads **Gitleaks** (when **`run_gitleaks`**), **Bandit**, **Trivy repo** (**`trivy`**), and **Trivy image** (**`trivy-image`**) SARIF via **`github/codeql-action/upload-sarif`** — caller workflows need **`permissions: security-events: write`** on the job that **`uses`** this workflow (see example). Upload steps use **`continue-on-error: true`** so missing **GitHub Advanced Security** does not fail the pipeline.
- **Pip hash pinning:** The **pytest** *tool* install uses **`pip --require-hashes`** when a matching **`constraints/*.txt`** exists in the **pytest** composite; other versions log a **warning**. **Ruff** in CI comes from **pre-commit** (hook envs), not from **`ci.yml`** composites. The **application** graph from **`pip install -r`** (**`pytest_requirements_file`**) is **not** hash-verified unless that file contains **PEP 503** hashes (e.g. **`pip-compile --generate-hashes`**) — or you set **`pytest_app_require_hashes: true`** on **`ci.yml`**. **Lockfile enforcement** uses **[`install-pip-tools-hashed`](.github/actions/install-pip-tools-hashed/action.yml)** only when **`pip_tools_version`** is **exactly `7.5.3`**; any other value uses **plain** **`pip install pip-tools==…`**. Set **`pip_tools_require_hashed_install: true`** with **`enforce_pip_tools_lockfile: true`** to **fail** if **`pip_tools_version`** is not **`7.5.3`** until you extend bundled constraints (see **[`CI_RISK_REGISTER.md`](CI_RISK_REGISTER.md)**). **reusable-actionlint** uses **[`install-pyyaml-hashed`](.github/actions/install-pyyaml-hashed/action.yml)**. **Bandit** (**`sast-bandit`**) and **pip-audit** (**`pip-audit-scan-action`**) use the same pattern: hashed installs for their **default** versions bundled in those repos; custom **`bandit_version`** / **`pip_audit_version`** or missing constraint files fall back with **`::warning`**. Regenerate: **[`scripts/refresh-pip-constraints.sh`](scripts/refresh-pip-constraints.sh)** here; **`sast-bandit`** / **`scan-pip-audit`** each ship **`scripts/refresh-pip-constraints.sh`** for their constraint files.
...
- **Pre-commit vs CI:** Every **caller** of **`ci.yml`** should commit **`.pre-commit-config.yaml`** at the **repo root** (copy from this repo’s template and adjust). **`ci.yml`** runs **`pre-commit run --all-files`** by default (**`pre-commit-check`** job) — same as local **`pre-commit run --all-files`** before push. **[`.pre-commit-config.yaml`](.pre-commit-config.yaml)** here includes **[`pre-commit-hooks`](https://github.com/pre-commit/pre-commit-hooks)** (trailing whitespace, EOF, **`check-yaml`** excluding **`.github/workflows/`** because **`on:`** is invalid YAML 1.1, JSON, merge conflicts, line endings, private keys, **`debug-statements`**) plus **Black** (format) + **Ruff** (lint) + **Gitleaks**; bump **`gitleaks`** hook **`rev:`** in **`.pre-commit-config.yaml`** when you upgrade **Gitleaks**. **Do not** add **`ruff-format`** alongside **Black** (two formatters fight). Install **`pip install pre-commit`** then **`pre-commit install`**. Bump **`psf/black`** and **`astral-sh/ruff-pre-commit`** **`rev:`** when you upgrade; run **`pre-commit autoupdate`** for **pre-commit-hooks**. **Monorepo:** run from the directory that contains **`.git`**. This repo also runs **[`.github/workflows/pre-commit.yml`](.github/workflows/pre-commit.yml)** on push/PR so **`main`** is checked even when the full **`dogfood-ci`** graph does not run.

### Reusable actionlint (for other repositories)

**This repo** uses the **actionlint** pre-commit hook (see **[`.pre-commit-config.yaml`](.pre-commit-config.yaml)**). [`reusable-actionlint.yml`](.github/workflows/reusable-actionlint.yml) is for **callers** that do not use **`ci.yml`** / pre-commit; it runs **actionlint** on workflow files and optionally validates YAML for:
...
2. **Pip hash constraints:** If you change default **`pytest_version`**, **`pip_tools_version`** (default **7.5.3**), or the **PyYAML** version bundled in **[`install-pyyaml-hashed`](.github/actions/install-pyyaml-hashed/)**, run **[`scripts/refresh-pip-constraints.sh`](scripts/refresh-pip-constraints.sh)** and commit the updated **`constraints*.txt`** files. If you bump **Ruff** in **[`.pre-commit-config.yaml`](.pre-commit-config.yaml)** and use the **[Ruff composite](.github/actions/ruff/README.md)** elsewhere, refresh **[`.github/actions/ruff/constraints/`](.github/actions/ruff/constraints/)** with the same script. Skipping this keeps CI working but may emit **`::warning`** and use unhashed **`pip install`** where constraints are missing (see **[`ORG_PORTABILITY.md`](ORG_PORTABILITY.md)** and **`MAINTAINERS.md`**).
3. In [`.github/workflows/ci.yml`](.github/workflows/ci.yml), set **`thadiust/workflow-python/.github/actions/pytest@v1.x.y`** to match the tag you are about to create (so **`ci.yml@v1.x.y`** and the composite resolve to the same commit). Pin **`thadiust/trivy-scan`**, **`secrets-gitleaks`**, **`sast-bandit`**, and **`pip-audit-scan-action`** to the semver tags you want that release to ship.
4. Commit, create the annotated tag **`v1.x.y`**, push **`main`** and **`git push origin v1.x.y`**.
5. On **`main`**, follow up with a commit that sets **pytest**, **Trivy**, **`secrets-gitleaks`**, **`sast-bandit`**, and **`pip-audit-scan-action`** back to **`@main`** so day-to-day CI floats latest (the **tag** remains a frozen snapshot; verify with **`git show v1.x.y:.github/workflows/ci.yml`**). When cutting a release tag, pin those **`uses:`** lines to **`@v…`** as needed for reproducibility.
...
| `python_version` | string | `3.11` | Python version for pre-commit, pytest, Bandit, and pip-audit jobs (Trivy is a standalone binary; this input does not affect Trivy). |
...
| `run_pre_commit` | boolean | `true` | If `false`, the **pre-commit** job is skipped (not recommended — you lose **Gitleaks** hook, **Ruff**, and other hooks in **`ci.yml`** unless you replace them). |
| `run_gitleaks` | boolean | `true` | If `false`, skip **`gitleaks-scan`** (full-history **`secrets-gitleaks`** + SARIF). **Pre-commit** **Gitleaks** hook still runs when **`run_pre_commit`** is **`true`**. |
...
| `run_docker_build` | boolean | `false` | If `true`, build a Docker image only after **pre-commit**, **Gitleaks** (job), **pytest**, **Trivy repo**, **Bandit**, and **pip-audit** have each succeeded or been skipped (toggle off), then upload a tarball for **`trivy-image-scan`**. |
...
| `upload_code_scanning` | boolean | `true` | If `true`, **Gitleaks** (when **`run_gitleaks`**), **Bandit**, **Trivy repo**, and **Trivy image** jobs write SARIF and upload to **Code Scanning** (Security tab / PR). Requires **`security-events: write`** on the **caller** job. Fork PRs from outside contributors may not upload (token limits). |
