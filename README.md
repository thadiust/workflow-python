...
3. **[Trivy](https://github.com/aquasecurity/trivy)** (**Wave 1** repository **fs** + **config**) via [`thadiust/trivy-scan`](https://github.com/thadiust/trivy-scan) (**`uses: …/trivy-scan@main`** on branch **main**). Job **`trivy-repo-scan`** **`needs`** **Ruff** and **pytest** (same gate as **Gitleaks**), so it runs **in parallel with Gitleaks** once lint/tests are green. Optional **`docker-build`** **`needs`** **Ruff**, **pytest**, **`gitleaks-scan`**, **`trivy-repo-scan`**, **`bandit-scan`**, and **`pip-audit-scan`** — the image is built only after that whole DevSecOps wave succeeds (each job may be **skipped** only when its toggle is off). **`trivy-image-scan`** **`needs`** **`docker-build`** only — a **leaf**. Toggle with **`run_trivy`**, **`run_docker_build`**, **`run_trivy_image_scan`**, and **`trivy_*`** inputs.
...
5. **[Bandit](https://github.com/pycqa/bandit)** via [`thadiust/sast-bandit`](https://github.com/thadiust/sast-bandit) (**`…/sast-bandit@main`**) and **[pip-audit](https://github.com/pypa/pip-audit)** via [`thadiust/pip-audit-scan-action`](https://github.com/thadiust/pip-audit-scan-action) (**`…/pip-audit-scan-action@main`**) — **SAST** and **dependency vulnerabilities**. **`bandit-scan`** and **`pip-audit-scan`** each **`needs`** **`ruff-lint`** and **`pytest-test`** only, so they run **in parallel with Gitleaks and Trivy repo** after lint/tests (typical **build/test + concurrent security** stage).

**Gitleaks**, **Trivy repo**, **Bandit**, and **pip-audit** all start only after **Ruff** and **pytest** succeed (or are skipped via toggles). They do **not** depend on each other, so they fan out in parallel. **`run_pytest: false`** skips the pytest job so downstream jobs still run when other gates pass. Toggle jobs with **`run_ruff`**, **`run_pytest`**, **`run_trivy`**, **`run_gitleaks`**, **`run_bandit`**, **`run_pip_audit_scan`**, **`run_docker_build`**, and **`run_trivy_image_scan`**. If **every** job is disabled, the workflow has no jobs and GitHub will reject the run — leave at least one enabled.

This layout runs **Ruff** and **pytest** first; then **Gitleaks**, **Trivy repo**, **Bandit**, and **pip-audit** **in parallel**. **Docker build** (optional) runs only after **all** of those complete successfully (or were turned off); **Trivy image** (optional) runs after **Docker build** and is **not** a dependency for anything else.
...
- **Tests before security scans (deliberate):** **Gitleaks**, **Trivy repo**, **Bandit**, and **pip-audit** run only after **Ruff** and **pytest** succeed (or those jobs are skipped via toggles). If **pytest fails**, they **do not run**. Those four jobs are **in parallel** with each other (no **Bandit**/ **pip-audit** wait on **Gitleaks**). **Docker build** waits until **Gitleaks**, **Trivy repo**, **Bandit**, and **pip-audit** each **succeed** or are **skipped** because their toggle is off — so **no image** if secrets, repo Trivy, SAST, or SCA failed. **Trivy image** is a separate **leaf** job.
- **Parallel jobs vs minutes:** **`runner-info`** logs documented **`runner.*`** fields (`os`, `arch`, `environment`, `name`) once — **`runner.environment`** shows **`github-hosted`** vs **`self-hosted`**. **Ruff** and **pytest** run in parallel when both are enabled. Then up to **four** security jobs can run at once. Optional **Docker** / **Trivy image** add more runners when enabled. **Billed minutes** sum across all jobs. Disable jobs you do not need via inputs to save time.
...
| `run_docker_build` | boolean | `false` | If `true`, build a Docker image only after **Gitleaks**, **Trivy repo**, **Bandit**, and **pip-audit** have each succeeded or been skipped (toggle off), then upload a tarball for **`trivy-image-scan`**. |
