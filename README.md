...
**Lint in this repo:** [`.github/workflows/actionlint.yml`](.github/workflows/actionlint.yml) calls **[`reusable-actionlint.yml`](.github/workflows/reusable-actionlint.yml)**; **[`.github/workflows/pre-commit.yml`](.github/workflows/pre-commit.yml)** runs **[pre-commit](https://pre-commit.com/)** (**Ruff** + **Gitleaks**) on **`main`** pushes and PRs — mirror with **`pre-commit run --all-files`** before you push. Other repositories can call **`thadiust/workflow-python/.github/workflows/reusable-actionlint.yml@main`** (solo dev) or **`@v…`** when you want a frozen snapshot. User-visible changes by tag are listed in **[CHANGELOG.md](CHANGELOG.md)**.
...
- **Pre-commit vs CI:** **[pre-commit](https://pre-commit.com/)** is **required** for this repo. **[`.pre-commit-config.yaml`](.pre-commit-config.yaml)** includes **[`pre-commit-hooks`](https://github.com/pre-commit/pre-commit-hooks)** (trailing whitespace, EOF, **`check-yaml`** excluding **`.github/workflows/`** because **`on:`** is invalid YAML 1.1, JSON, merge conflicts, line endings, private keys, **`debug-statements`**) plus **Ruff** + **Gitleaks** aligned with **`ci.yml`** defaults. **Formatting:** use **`ruff-format`** only — **not** Black (would duplicate / disagree with Ruff). Install **`pip install pre-commit`** then **`pre-commit install`**; before push, **`pre-commit run --all-files`** (same as **[`.github/workflows/pre-commit.yml`](.github/workflows/pre-commit.yml)**). Bump hook **`rev:`** for **Ruff** / **Gitleaks** when **`ci.yml`** defaults change; run **`pre-commit autoupdate`** for **pre-commit-hooks**. **Monorepo:** run from **`workflow-python/`** (needs **`.git`**).
