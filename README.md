...
3. **[Trivy](https://github.com/aquasecurity/trivy)** (**Wave 1** repository **fs** + **config**) via [`thadiust/trivy-scan`](https://github.com/thadiust/trivy-scan). **[`ci.yml`](.github/workflows/ci.yml)** pins **`uses: thadiust/trivy-scan@<commit>`** (not **`@main`**) so editors and CI resolve the same action inputs (**`scan_kind`**, **`image_ref`**, …); bump that SHA when you adopt a newer **trivy-scan** release. Job **`trivy-repo-scan`** **`needs`** **Ruff** and **pytest** (same gate as **Gitleaks**), so it runs **in parallel with Gitleaks** once lint/tests are green. Optional **`docker-build`** **`needs`** those plus **`trivy-repo-scan`**. **`trivy-image-scan`** **`needs`** **`docker-build`** only — a **leaf**. Toggle with **`run_trivy`**, **`run_docker_build`**, **`run_trivy_image_scan`**, and **`trivy_*`** inputs.
...
**Bandit** and **pip-audit** each **`needs`** **`ruff-lint`**, **`gitleaks-scan`**, and **`pytest-test`**. If **pytest fails**, **Gitleaks**, **Trivy repo**, **Bandit**, and **pip-audit** **do not run**. **Trivy repo** uses the **same** Ruff/pytest gate as **Gitleaks** (then runs **beside** Gitleaks). If **Gitleaks fails**, Bandit and pip-audit are skipped. **`run_pytest: false`** skips the pytest job so downstream jobs still run when other gates pass. Toggle jobs with **`run_ruff`**, **`run_pytest`**, **`run_trivy`**, **`run_gitleaks`**, **`run_bandit`**, **`run_pip_audit_scan`**, **`run_docker_build`**, and **`run_trivy_image_scan`**. If **every** job is disabled, the workflow has no jobs and GitHub will reject the run — leave at least one enabled.

This layout runs **Ruff** and **pytest** first; then **Gitleaks** and **Trivy repo** in parallel; then **Bandit** and **pip-audit** (parallel with each other). **Docker build** (optional) runs after **Trivy repo**; **Trivy image** (optional) runs after **Docker build** and is **not** a dependency for anything else.
...
- **Tests before security scans (deliberate):** **Gitleaks**, **Trivy repo**, **Bandit**, and **pip-audit** run only after **Ruff** and **pytest** succeed (or those jobs are skipped via toggles). If **pytest fails**, they **do not run**. **Docker build** waits on **Trivy repo** as well (so repository scanning completes before the image is built when **`run_trivy`** is on). **Trivy image** is a separate **leaf** job and does not block Bandit, pip-audit, or Gitleaks.
- **Parallel jobs vs minutes:** **`runner-info`** logs documented **`runner.*`** fields (`os`, `arch`, `environment`, `name`) once — **`runner.environment`** shows **`github-hosted`** vs **`self-hosted`**. **Ruff** and **pytest** run in parallel when both are enabled. **Gitleaks** and **Trivy repo** start after both finish (**success or skipped**), **in parallel with each other**. After Gitleaks, **Bandit** and **pip-audit** run **at the same time**. Optional **Docker** / **Trivy image** add more runners when enabled. **Billed minutes** sum across all jobs. Disable jobs you do not need via inputs to save time.
...
4. On **`main`**, follow up with a commit that sets **Ruff**, **pytest**, **`secrets-gitleaks`**, **`sast-bandit`**, and **`pip-audit-scan-action`** back to **`@main`** so day-to-day CI floats latest. Keep **`thadiust/trivy-scan@<commit>`** pinned to a SHA (bump when the action changes); the **tag** remains a frozen snapshot — verify with **`git show v1.x.y:.github/workflows/ci.yml`**. When cutting a release tag, pin other **`uses:`** lines to **`@v…`** as needed for reproducibility.
