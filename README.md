...
**Lint in this repo:** [`.github/workflows/actionlint.yml`](.github/workflows/actionlint.yml) calls **[`reusable-actionlint.yml`](.github/workflows/reusable-actionlint.yml)**; **[`.github/workflows/pre-commit.yml`](.github/workflows/pre-commit.yml)** runs **[pre-commit](https://pre-commit.com/)** on **`main`** pushes and PRs (same hooks as **`ci.yml`**’s **`pre-commit-check`** job for consumers). **Before every push:** **`pre-commit run --all-files`** so CI matches your machine. Other repositories can call **`thadiust/workflow-python/.github/workflows/reusable-actionlint.yml@main`** (solo dev) or **`@v…`** when you want a frozen snapshot. User-visible changes by tag are listed in **[CHANGELOG.md](CHANGELOG.md)**.
