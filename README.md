...
4. **[Gitleaks](https://github.com/gitleaks/gitleaks)** via [`thadiust/secrets-gitleaks`](https://github.com/thadiust/secrets-gitleaks) (**`uses: …/secrets-gitleaks@v1.0.3`** on branch **main** — tag releases in that repo to bump). Secrets; **full git history** — checkout uses `fetch-depth: 0`. **`gitleaks-scan`** **`needs`** **`ruff-lint`** and **`pytest-test`**. See [**Removed the file, but CI still fails?**](https://github.com/thadiust/secrets-gitleaks/blob/main/README.md#removed-the-file-but-ci-still-fails) in the action README.
5. After Gitleaks: **[Bandit](https://github.com/pycqa/bandit)** via [`thadiust/sast-bandit`](https://github.com/thadiust/sast-bandit) (**`…/sast-bandit@v1.0.2`**) and **[pip-audit](https://github.com/pypa/pip-audit)** via [`thadiust/pip-audit-scan-action`](https://github.com/thadiust/pip-audit-scan-action) (**`…/pip-audit-scan-action@v1.0.1`**) — dependency **vulnerabilities** (in parallel with each other).
...
- **Supply chain:** **Consumers should reference this workflow by semver tag** (for example **`@v1.0.8`**) so upgrades are explicit and reproducible. This project does not require pinning to commit SHAs. **`@main` is intended for development/testing**, not the default for app repositories. **Note:** Each job’s **`uses:`** resolves independently. On **semver tags**, **Ruff** and **pytest** use the **same tag** as **`ci.yml`**. On branch **`main`**, **Ruff** and **pytest** use **`@main`** so CI exercises the latest local composites. **`thadiust/trivy-scan`** is referenced at **`@main`** on branch **`main`** (tag that action repo for stricter pinning). **`secrets-gitleaks`** is pinned at **`@v1.0.3`**; **`sast-bandit`** at **`@v1.0.2`**; **`pip-audit-scan-action`** at **`@v1.0.1`** (see [`ci.yml`](.github/workflows/ci.yml)). **`upload_code_scanning`** (default **`true`**) uploads **Gitleaks**, **Bandit**, and **Trivy** SARIF via **`github/codeql-action/upload-sarif`** — caller workflows need **`permissions: security-events: write`** on the job that **`uses`** this workflow (see example). Upload steps use **`continue-on-error: true`** so missing **GitHub Advanced Security** does not fail the pipeline.
...
Callers: **`uses: thadiust/workflow-python/.github/workflows/reusable-actionlint.yml@v1.0.8`** (or newer tag).
...
jobs:
  security:
    uses: thadiust/workflow-python/.github/workflows/ci.yml@v1.0.8
    with:
      working_directory: "."
...
For controlled upgrades, call **`uses: …/ci.yml@v1.0.8`** (or another tag) instead of **`@main`**; bump the tag when you intentionally adopt a new release. That tag’s **`ci.yml`** pins **Ruff/pytest** to the same tag and lists **`thadiust/*`** versions (see supply chain note above).
