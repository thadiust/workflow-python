...
1. **[pre-commit](https://pre-commit.com/)** (**`pre-commit run --all-files`**) via **`pre-commit/action`** — uses the **caller** repo’s **`.pre-commit-config.yaml`**. That template puts the **[Gitleaks](https://github.com/gitleaks/gitleaks)** hook first (**“Detect hardcoded secrets”** in logs), then **pre-commit-hooks**, **Black**, **`ruff`** with **`--fix`**, etc.; **`fail_fast: true`**. **There is no separate Gitleaks job in `ci.yml`** — secrets are checked **only** here (same as local **`pre-commit install`**). For **full-history** scanning or **SARIF** uploads via **[`secrets-gitleaks`](https://github.com/thadiust/secrets-gitleaks)**, add your **own** workflow job. See [**Removed the file, but CI still fails?**](https://github.com/thadiust/secrets-gitleaks/blob/main/README.md#removed-the-file-but-ci-still-fails). Toggle **`run_pre_commit`**. There is **no** separate **Ruff** Actions job — use **[`.github/actions/ruff`](.github/actions/ruff/README.md)** only if you want Ruff outside pre-commit.
2. **[pytest](https://pytest.org/)** via [`.github/actions/pytest`](.github/actions/pytest/README.md) — **`needs`** **`pre-commit-check`** so slow tests run after hooks. Toggle **`run_pytest`**.
3. **[Trivy](https://github.com/aquasecurity/trivy)** (**fs** + **config**) via [`thadiust/trivy-scan`](https://github.com/thadiust/trivy-scan) (**`…/trivy-scan@main`**). **`trivy-repo-scan`** **`needs`** **pre-commit** and **pytest**. Toggle **`run_trivy`** and **`trivy_*`**.
4. **[Bandit](https://github.com/pycqa/bandit)** and **[pip-audit](https://github.com/pypa/pip-audit)** via **`sast-bandit`** / **`pip-audit-scan-action`** — **`needs`** **pre-commit** and **pytest**; **in parallel with each other and with Trivy repo** after pytest. Toggle **`run_bandit`**, **`run_pip_audit_scan`**.
5. Optional **`docker-build`** then **`trivy-image-scan`** (leaf): **`docker-build`** **`needs`** **pre-commit**, **pytest**, **Trivy repo**, **Bandit**, **pip-audit** — image only after that wave (**`run_docker_build`**, **`run_trivy_image_scan`**, **`trivy_image_fail_on_findings`**, etc.).

**Toggle jobs** with **`run_pre_commit`**, **`run_pytest`**, **`run_trivy`**, **`run_bandit`**, **`run_pip_audit_scan`**, **`run_docker_build`**, **`run_trivy_image_scan`**. If **every** job is disabled, GitHub rejects the run — leave at least one enabled.

**DAG summary:** **pre-commit-check** (includes **Gitleaks** hook) → **pytest** → **Trivy repo ∥ Bandit ∥ pip-audit** → optional **Docker** → optional **Trivy image**.
...
|-----|----------------|----------------|
| pre-commit | **Hooks** (whitespace, YAML/JSON, **Gitleaks**, **Black**, **Ruff** (lint), etc.) — **secrets only here**, not a second CI job | hook failures in log |
...
**`ci.yml`** forwards **tool versions** (**`bandit_version`**, **`pip_audit_version`**) and **`bandit_targets`**. **Gitleaks** is configured only in **`.pre-commit-config.yaml`** (hook **`rev:`**). It still **does not** expose every composite knob (e.g. Bandit **`report_format`**, pip-audit **`report_file`**). For those, **fork** and extend **`workflow_call`** inputs or wrap a job.
...
- **Fast-then-slow (deliberate):** **`pre-commit-check`** runs first (hooks include **Gitleaks**). **pytest** waits on it (may be **skipped** only when **`run_pre_commit`** is off). **Trivy repo**, **Bandit**, and **pip-audit** wait on **pre-commit** + **pytest**, then run **in parallel**. **Docker build** waits on all of those (each **skipped** only when its toggle is off). **Trivy image** is a **leaf**.
- **Parallel jobs vs minutes:** **`runner-info`** logs **`runner.*`** once (toggle **`run_runner_info: false`** to drop that job). **pre-commit**; then **pytest**; then up to **three** scanners in parallel. Optional **Docker** / **Trivy image** add runners. Disable jobs via inputs to save minutes.
...
3. In [`.github/workflows/ci.yml`](.github/workflows/ci.yml), set **`thadiust/workflow-python/.github/actions/pytest@v1.x.y`** to match the tag you are about to create (so **`ci.yml@v1.x.y`** and the composite resolve to the same commit). Pin **`thadiust/trivy-scan`**, **`sast-bandit`**, and **`pip-audit-scan-action`** to the semver tags you want that release to ship.
4. Commit, create the annotated tag **`v1.x.y`**, push **`main`** and **`git push origin v1.x.y`**.
5. On **`main`**, follow up with a commit that sets **pytest**, **Trivy**, **`sast-bandit`**, and **`pip-audit-scan-action`** back to **`@main`** so day-to-day CI floats latest (the **tag** remains a frozen snapshot; verify with **`git show v1.x.y:.github/workflows/ci.yml`**). When cutting a release tag, pin those **`uses:`** lines to **`@v…`** as needed for reproducibility.
...
| `run_pre_commit` | boolean | `true` | If `false`, the **pre-commit** job is skipped (not recommended — you lose **Gitleaks**, **Ruff**, and other hooks in **`ci.yml`** unless you replace them). |
...
| `run_docker_build` | boolean | `false` | If `true`, build a Docker image only after **pre-commit**, **pytest**, **Trivy repo**, **Bandit**, and **pip-audit** have each succeeded or been skipped (toggle off), then upload a tarball for **`trivy-image-scan`**. |
...
| `upload_code_scanning` | boolean | `true` | If `true`, **Bandit**, **Trivy repo**, and **Trivy image** jobs write SARIF and upload to **Code Scanning** (Security tab / PR). **Gitleaks** has no SARIF in **`ci.yml`** (hook-only). Requires **`security-events: write`** on the **caller** job. Fork PRs from outside contributors may not upload (token limits). |
