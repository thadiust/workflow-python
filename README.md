...
- **Pre-commit vs CI:** **[pre-commit](https://pre-commit.com/)** is **required** for this repo: **[`.pre-commit-config.yaml`](.pre-commit-config.yaml)** pins **Ruff** + **Gitleaks** to the same major versions as **`ci.yml`** defaults. Install with **`pip install pre-commit`** then **`pre-commit install`**; before push, run **`pre-commit run --all-files`** (matches **[`.github/workflows/pre-commit.yml`](.github/workflows/pre-commit.yml)** on GitHub). When you bump **`ruff_version`** or **`gitleaks_version`** in **`ci.yml`**, update hook **`rev:`** tags in **`.pre-commit-config.yaml`** so local and CI stay aligned. **Monorepo workspace:** if the parent folder (e.g. **`security-pipeline`**) is **not** a Git root, **`cd`** into **`workflow-python/`** before **`pre-commit`** — it needs a **`.git`** directory.
