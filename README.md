...
**Dependency Review** is **not** part of the **`ci.yml`** DAG. For **default PR onboarding**, call **[`python-pr-suite.yml`](.github/workflows/python-pr-suite.yml)** (**Dependency Review** ∥ **`ci.yml`**) or copy **[`examples/consumer-pull-request-ci.yml`](examples/consumer-pull-request-ci.yml)**. A **minimal** two-job template is also below.
