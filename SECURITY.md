...
- **CI threat posture** (untrusted PR code, hash coverage, Trivy policy, artifacts, Dependency Review): **[`CI_RISK_REGISTER.md`](CI_RISK_REGISTER.md)**. **Company / platform rollout:** **[`COMPANY_RUNBOOK.md`](COMPANY_RUNBOOK.md)**. Workflows here avoid **`pull_request_target`** for running untrusted code with base-repo secrets; integration uses **`pull_request`**, **`workflow_call`**, and **`workflow_dispatch`** as documented in the README.
